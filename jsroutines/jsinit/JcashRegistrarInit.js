...
export const verifyJNTConnection = async (jcashRegistrarAddress, jntControllerAddress, ethAccounts: TxConfig.EthereumAccounts, transferCost) => {
  ...
  const isVerified4 = (receivedTransferEthPrice.toString(16) === transferCost.replace('0x', ''));
  if (isVerified4 !== true) {
  ...
  const isVerified5 = (receivedTransferTokenPrice.toString(16) === transferCost.replace('0x', ''));
  if (isVerified5 !== true) {
