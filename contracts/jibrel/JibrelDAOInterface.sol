...
contract JibrelDAOInterface {
    ...
    function addProposal(string _description, bytes _transactionBytecode) external returns (uint256 _proposalId);
    function voteForProposal(uint256 _proposalId, bool _inSupport) external;
    ...
    // get proposal info
    function getProposalDescription(uint256 _proposalId) external returns (string _proposalDescription);
    function getProposalBytecode(uint256 _proposalId) external returns (bytes _bytecode);
    function isProposalResolved(uint256 _proposalId) external returns (bool _isResolved);
    function isProposalApproved(uint256 _proposalId) external returns (bool _isApproved);
