...
contract JibrelAPIInterface {
  ...
  function getBODC() external constant returns (address);
  function getJibrelDAO() external constant returns (address);
  function getInvestorRegistry() external constant returns (address);
  function getCryDRRegistry() external constant returns (address);
}
...
contract JibrelAPI is JibrelAPIInterface, Manageable {
  ...
  function setBODC(address _newAddress) checkAddress(_newAddress) external onlyAllowedManager('set_bodc') {
    BODC = _newAddress;
    BODCChangedEvent(_newAddress);
  }

  function setJibrelDAO(address _newAddress) checkAddress(_newAddress) external onlyAllowedManager('set_jibrel_dao') {
    JibrelDAO = _newAddress;
    JibrelDAOChangedEvent(_newAddress);
  }

  function setInvestorRegistry(address _newAddress) checkAddress(_newAddress) external onlyAllowedManager('set_investor_repo') {
    InvestorRegistry = _newAddress;
    InvestorRegistryChangedEvent(_newAddress);
  }

  function setCryDRRegistry(address _newAddress) checkAddress(_newAddress) external onlyAllowedManager('set_crydr_repo') {
