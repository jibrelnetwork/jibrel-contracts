...
global.contract('Integration tests - JUSD', (accounts) => {
  ...
  global.it('should test minting of the tokens', async () => {
    ...
    global.assert.strictEqual(accountBalance.toNumber(), 0, 'expected that a new user has zero balance');
    ...
    global.assert.strictEqual(accountBalance.toNumber(), 10 ** 18, 'expected that user has 1 JUSD');
    ...
    await CrydrControllerMintableInterfaceJSAPI.burn(JUSDControllerAddress,
                                                     ...
                                                     10 ** 17);
    accountBalance = await ERC20InterfaceJSAPI.balanceOf(JUSDViewERC20Address, ethAccounts.testInvestor1);
    global.assert.strictEqual(accountBalance.toNumber(), 9 * (10 ** 17), 'expected that user has 0.9 JUSD');
    ...
    await CrydrControllerMintableInterfaceJSAPI.burn(JUSDControllerAddress,
                                                     ...
                                                     9 * (10 ** 17));
    accountBalance = await ERC20InterfaceJSAPI.balanceOf(JUSDViewERC20Address, ethAccounts.testInvestor1);
    global.assert.strictEqual(accountBalance.toNumber(), 0, 'expected that user has 0 JUSD');
  });
