...
global.contract('CrydrViewERC20Validatable', (accounts) => {
  ...
  global.it('should test that contract works as expected', async () => {
    ...
    global.assert.notStrictEqual(CrydrViewERC20ValidatableContract.address, '0x0000000000000000000000000000000000000000');

    global.console.log(`\tCrydrControllerSingleLicenseContract: ${CrydrERC20ValidatableMockContract.address}`);
    global.assert.notStrictEqual(CrydrERC20ValidatableMockContract.address, '0x0000000000000000000000000000000000000000');
    ...
    global.assert.strictEqual(controllerAddress, CrydrERC20ValidatableMockContract.address,
    ...
    global.assert.strictEqual(isApprovedSpendingAllowed, true);
    ...
    global.assert.strictEqual(isTransferFromAllowed, true);
  });
});
